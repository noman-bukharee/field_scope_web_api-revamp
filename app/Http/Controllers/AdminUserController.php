<?php namespace App\Http\Controllers;
	...
	class AdminUserController extends \crocodicstudio\crudbooster\controllers\CBController {
	    ...
	    public function cbInit() {
            ...
            $this->form[] = ['label' => 'Image Url', 'name' => 'image_url', 'type' => 'filemanager', 'validation' => 'required|min:1|max:255', 'width' => 'col-sm-10'

                , 'callback' => function ($row) {
                    $src = '';
                    if (!empty($row->image_url)) {
                        $src =  'uploads/user/' . $row->image_url . 'test';
                    } else {
			...
			//$this->form[] = ['label'=>'Mobile No','name'=>'mobile_no','type'=>'text','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			//$this->form[] = ['label'=>'Email','name'=>'email','type'=>'email','validation'=>'required|min:1|max:255|email|unique:user','width'=>'col-sm-10'];
			//$this->form[] = ['label'=>'Image Url','name'=>'image_url','type'=>'textarea','validation'=>'required|min:1|max:255','width'=>'col-sm-10'];
			# OLD END FORM
	    ...
	    */
	    public function hook_query_index(&$query) {
	        //Your code here
            $query->where('company_group_id',NULL);
	    }
